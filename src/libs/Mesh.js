module.exports = function Mesh(gl) {

  var utils = require('./webgl-utils.js')(gl)
    , vmath = require('./vector-math.js')
	...
	this.init = function(jsonstring) {
		...
		for (var m in mesh.materials) {
			...
			var prog = utils.loadProgram(material.vertexshader, material.fragmentshader, function(prog) { that.programLoaded(prog) })
	...
	this.load = function(file, callback) {
		...
		var that = this
		utils.loadFile(file, function(x) { that.init(x) }, false, true)
	}

	this.setMatrixUniforms = function(program) {
		gl.uniformMatrix4fv(program.mMatrixUniform, false, vmath.modelMatrix().d)
		gl.uniformMatrix4fv(program.pMatrixUniform, false, vmath.projectionMatrix().d)
		gl.uniformMatrix4fv(program.vMatrixUniform, false, vmath.viewMatrix().d)
	}
